...
// EnergiNet type specifies the type of energi network
type EnergiNet uint32
...
// EnergiRPC is an interface to JSON-RPC energi service.
...
// NewEnergiRPC returns new EnergiRPC instance.
func NewEnergiRPC(config json.RawMessage, pushHandler func(bchain.NotificationType)) (bchain.BlockChain, error) {
	...
	go func() {
		for {
			...
			if glog.V(2) {
				glog.Info("rpc: new tx ", t)
			}
			s.Mempool.AddTransactionToMempool(string(t))
...
// Initialize initializes energi rpc interface
...
func (b *EnergiRPC) InitializeMempool(addrDescForOutpoint bchain.AddrDescForOutpointFunc, onNewTxAddr bchain.OnNewTxAddrFunc) error {
	if b.Mempool == nil {
...
func (b *EnergiRPC) subscribeEvents() error {
	...
	if err := b.subscribe(func() (*rpc.ClientSubscription, error) {
		...
		if err != nil {
			return nil, errors.Annotatef(err, "EnergiSubscribe newPendingTransactions")
		}
...
// Shutdown cleans up rpc interface to energi
...
// GetSubversion returns empty string, energi does not have subversion
...
func (b *EnergiRPC) GetBestBlockHeight() (uint32, error) {
	...
	return uint32(h.Number.ToInt().Int64()), nil
}
...
func (b *EnergiRPC) nrgHeaderToBlockHeader(h *rpcHeader) (*bchain.BlockHeader, error) {
	height, err := nrgNumber(h.Number)
	if err != nil {
		return nil, err
	}
	...
	time, err := nrgNumber(h.Time)
	if err != nil {
		return nil, err
	}
	size, err := nrgNumber(h.Size)
	if err != nil {
		return nil, err
	}
...
func (b *EnergiRPC) GetBlockHeader(hash string) (*bchain.BlockHeader, error) {
	...
	return b.nrgHeaderToBlockHeader(&h)
}
...
func (b *EnergiRPC) GetBlock(hash string, height uint32) (*bchain.Block, error) {
	...
	bbh, err := b.nrgHeaderToBlockHeader(&head)
	if err != nil {
		return nil, errors.Annotatef(err, "hash %v, height %v", hash, height)
	}
	// get ERC20 events
	...
	for i := range body.Transactions {
		...
		btx, err := b.Parser.nrgTxToTx(tx, &rpcReceipt{Logs: logs[tx.Hash]}, bbh.Time, uint32(bbh.Confirmations), true)
		if err != nil {
...
func (b *EnergiRPC) GetBlockInfo(hash string) (*bchain.BlockInfo, error) {
	...
	bch, err := b.nrgHeaderToBlockHeader(&head)
	if err != nil {
		return nil, err
	}
...
func (b *EnergiRPC) GetTransaction(txid string) (*bchain.Tx, error) {
	...
	if tx.BlockNumber == "" {
		// mempool tx
		btx, err = b.Parser.nrgTxToTx(tx, nil, 0, 0, true)
		if err != nil {
		...
		var time int64
		if time, err = nrgNumber(ht.Time); err != nil {
			return nil, errors.Annotatef(err, "txid %v", txid)
		}
		...
		n, err := nrgNumber(tx.BlockNumber)
		if err != nil {
			return nil, errors.Annotatef(err, "txid %v", txid)
		}
		...
		btx, err = b.Parser.nrgTxToTx(tx, &receipt, time, confirmations, true)
		if err != nil {
			return nil, errors.Annotatef(err, "txid %v", txid)
		}
...
// GetChainParser returns energi BlockChainParser
