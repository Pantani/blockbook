...
// EnergiTypeAddressDescriptorLen - in case of EthereumType, the AddressDescriptor has fixed length
const EnergiTypeAddressDescriptorLen = 20

// EnergiAmountDecimalPoint defines number of decimal points in Energi amounts
const EnergiAmountDecimalPoint = 18
...
func NewEnergiParser(b int) *EnergiParser {
	return &EnergiParser{&bchain.BaseParser{
		...
		AmountDecimalPoint:   EnergiAmountDecimalPoint,
	}}
}
...
func nrgNumber(n string) (int64, error) {
	if len(n) > 2 {
...
func (p *EnergiParser) nrgTxToTx(tx *rpcTransaction, receipt *rpcReceipt, blocktime int64, confirmations uint32, fixEIP55 bool) (*bchain.Tx, error) {
	txid := tx.Hash
	var (
		fa, ta []string
		err    error
	...
	return &bchain.Tx{
		...
		Time: blocktime,
		Txid: txid,
		Vin:  []bchain.Vin{{Addresses: fa}},
		Vout: []bchain.Vout{
			{
				N:            0, // there is always up to one To address
				ValueSat:     *vs,
				ScriptPubKey: bchain.ScriptPubKey{Addresses: ta},
			},
		},
		CoinSpecificData: ct,
	}, nil
...
func (p *EnergiParser) GetAddrDescFromAddress(address string) (bchain.AddressDescriptor, error) {
	...
	if len(address) != EnergiTypeAddressDescriptorLen*2 {
		return nil, bchain.ErrAddressMissing
	}
...
func (p *EnergiParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	...
	if pt.Tx.Hash, err = hexDecode(r.Tx.Hash); err != nil {
		return nil, errors.Annotatef(err, "NrgHash %v", r.Tx.Hash)
	}
...
func (p *EnergiParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
	...
	tx, err := p.nrgTxToTx(&rt, rr, int64(pt.BlockTime), 0, false)
	if err != nil {
		return nil, 0, err
	}
